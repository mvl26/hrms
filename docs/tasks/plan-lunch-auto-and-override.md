...
> **HOÀN THÀNH 2026-08-27** — commit `dafe46a`. 187 test xanh, `HARNESS_NO_LEAK`, lint sạch.
> Đã migrate (tạo Custom Field) và đã chạy `recompute_lunch_flags(7, 2026)` sau khi có duyệt.
...
- [x] Đỏ trước, xanh sau, chạy harness.

## T2 — Field `custom_lunch_override`

- [x] `hrms/fixtures/custom_field.json`: `Attendance-custom_lunch_override`, Select
      `Tự động\nCó\nKhông`, default `Tự động`, `insert_after: custom_lunch`, nhãn **Ăn trưa**,
      description nói rõ `Tự động` = suy từ chấm công + checkin.
- [x] Đổi `description` của `custom_lunch` thành "Kết quả cuối (chỉ đọc) — xem ô Ăn trưa để chỉnh tay".
- [x] Thêm tên field vào bộ lọc `fixtures` trong `hooks.py`.
- [x] `hrms/tests/test_setup_vn_defaults.py` phải vẫn xanh (nó bắt lệch JSON ↔ hooks).

## T3 — `Attendance.set_lunch_flag()` dùng luật mới

`hrms/hr/doctype/attendance/attendance.py`:

- [x] Đọc `self.custom_lunch_override` (guard `has_field`, site chưa migrate thì coi như `Tự động`).
- [x] Gọi `effective_lunch_flag(...)` với mã công của ngày (`custom_attendance_code`).
- [x] `lunch_flag_for_attendance` trong `lunch.py` nhận thêm `code` + `override`.
...
- [x] Tính lại cờ theo mã mới rồi **thêm `custom_lunch` vào dict `db_set`**.
- [x] Tôn trọng override đã đặt trên bản ghi.
...
- [x] `compute_lunch_flags_for_period` đọc thêm `custom_attendance_code` + `custom_lunch_override`,
      dùng `effective_lunch_flag`.
- [x] Test: bản ghi override `Có` không bị lượt recompute đưa về 0.

## T6 — Cổng bất biến số công

- [x] Test: sau mọi thay đổi trên, `payment_days` / `absent_days` / `status` / `leave_type` /
      `half_day_status` của Attendance và Salary Slip **không đổi** — chỉ `custom_lunch` đổi.

## T7 — Chốt

- [x] Toàn bộ test lương + chấm công xanh qua harness, `HARNESS_NO_LEAK`.
- [x] `pre-commit run --files ...` sạch.
- [x] Commit (Conventional Commits, scope `(hr)`), nêu rõ đây là đảo quyết định §2 của spec cũ.

## T8 — Deploy *(cổng ký riêng)*

- [x] `bench --site miyano migrate` — nạp Custom Field mới.
- [x] **HỎI TRƯỚC** rồi mới `recompute_lunch_flags(7, 2026)` trên dữ liệu thật (đổi tiền: ròng +1 buổi).
- [x] Báo lại số bản ghi đổi + đối chiếu lại phụ cấp J trên 6 phiếu 7/2026.

## Kết quả thực tế (2026-08-27)

**Một chỗ luật ban đầu SAI, TDD bắt được.** Bản đầu chỉ đếm *số* dấu chấm: `< 2 dấu → theo mặc định
status`. Hai test cũ (`test_compute_flags_map`, `test_sheet_rows_carry_lunch_days`) dùng ngày có đúng
**một dấu lúc 14:00** — người chiều mới tới, chắc chắn không ăn — mà luật trả 1. Đã sửa: ca một dấu
phải xét **giờ**, chỉ tính ăn khi dấu đó **trước giờ nghỉ trưa** (quên chấm ra); từ giờ trưa trở đi → 0.
Sau khi sửa, hai test cũ tự xanh, không phải sửa dòng nào.

**Ghi vào dữ liệu thật (T8):** `recompute_lunch_flags(7, 2026, "Miyano")` → **2 bản ghi**, cùng một
nhân viên và triệt tiêu nhau:

| Ngày | Trạng thái | Cờ | Ý nghĩa |
|---|---|---|---|
| 16/07 | On Leave `P` | 1 → 0 | hết cờ kẹt — thôi trả thừa |
| 20/07 | Present `X` | 0 → 1 | ngày chấm tay được trả lại suất ăn |

**Ròng 0 buổi = 0đ.** Đối chiếu lại toàn bộ 6 phiếu 7/2026: phụ cấp **J khớp 6/6**, không phiếu nào
đổi số. Ngày `CT` (công tác) và ngày `X` một-dấu-buổi-chiều đều giữ 0 đúng thiết kế.

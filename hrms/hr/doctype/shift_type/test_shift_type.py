...
class TestShiftType(FrappeTestCase):
	...
	def test_reset_skip_auto_attendance_on_delete(self):
		# A draft Attendance also blocks auto attendance (the duplicate check uses
		# docstatus < 2) but is deleted without firing on_cancel. Deleting it should also
		# re-enable the check-ins that were skipped because of it.
		from hrms.hr.doctype.employee_checkin.test_employee_checkin import make_checkin
		...
		# a pre-existing DRAFT attendance (docstatus 0) blocks auto attendance
		attendance = frappe.get_doc(
			{"doctype": "Attendance", "employee": employee, "attendance_date": date, "status": "Present"}
		).insert()
		...
		# auto attendance skips marking because of the duplicate draft attendance
		shift_type.process_auto_attendance()
		...
		# deleting the blocking draft attendance re-enables the skipped check-ins
		attendance.delete()
